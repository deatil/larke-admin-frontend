...
Extension::routes(function ($router) {
    $router->namespace(config('frontend.route.namespace'))->group(function ($router) {
        ...
        $router->get('/frontend/menus', 'Frontend@menus')->name('larke-admin.frontend.menus');
        $router->get('/frontend/menus-tree', 'Frontend@menusTree')->name('larke-admin.frontend.menus-tree');
